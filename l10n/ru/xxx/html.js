...
function convertOutsideTextToHtml(page) {
    ...
    outsideSymbols.forEach(outsideSymbol => {
        domUtil.getSymbolTexts(outsideSymbol).forEach(override => {
...
function convertArtboardToHtml(page, artboard) {
    ...
    dom.find('SymbolInstance', copy).forEach(symbol => symbol.overrides.filter(override => override.affectedLayer.type === 'Text').forEach(override => override.value = ' '));
    const buffer = dom.export(copy, { output: false });
...
export {
    convertArtboardToHtml,
    convertOutsideTextToHtml,
    parseHtmlForText
};