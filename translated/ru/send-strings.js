...
async function uploadStrings(page, artboard) {
    ...
    const promises = artboards.filter(artboard => !translatedArtboards.includes(artboard.id)).map(artboard => uploadArtboard(uploadStorageApi, sourceFilesApi, projectFiles, page, artboard, projectId, directory.data.id));
...
async function uploadArtboard(uploadStorageApi, sourceFilesApi, projectFiles, page, artboard, projectId, directoryId) {
    ...
    const file = projectFiles.data.map(f => f.data).find(f => f.name === fileName);
